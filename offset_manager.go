...
func (om *OffsetManager) Add(partition int32, offset int64) error {
	...
	if om.currentOffsetMap[partition]-om.committedOffsetMap[partition] > om.commitThreshold {
		offsetReq := &sarama.OffsetCommitRequest{ConsumerGroup: om.consumerGroup, ConsumerID: om.consumerID, Version: 2, RetentionTime: 60 * 60 * 24}
		...
		if _, err := om.broker.CommitOffset(offsetReq); err != nil {
			...
			log.Printf("Commited offset for partition:%d consumergroup:%s consumerid:%s offset:%d", partition, om.consumerGroup, om.consumerID, offset)
